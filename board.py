...
class Board:
  ...
  def get_board_info(self, move, pieces_table_copy, verbose = False):
    ...
    lines_cleared = self.get_state_lines_cleared(simulated_pieces_table, coords, hard_drop_value)
    ...
    #return [landing_height / 20, hard_drop_value / 20, board_height / 20, bumpiness / 100, holes / 100, lines_cleared / 16, \
    #  row_transitions / 100, col_transitions / 100]

  def get_state_lines_cleared(self, board, coords, hard_drop_value):
    """Gets the number of lines cleared when a piece is hard dropped.
  ...
  def get_state_transitions(self, board):
    ...
    return row_transitions / 190, col_transitions / 190
